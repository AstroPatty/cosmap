...
class cosmapOutputParser(ABC):
    ...
    be written to a file, database, or other output. For example, a csv parser takes a dictionary
    of key-value pairs, and turns it into a pandas DataFrame. A parser is combined with an 
    output writer, which actually handles writing the data to disk.

    The parser should keep a working copy of any output it has recieved, unless it is explicitly cleared.
    This data may or may not be in the same same format as the output that is written to disk, as long as
    the "get" function returns an object of the type specified by the output_format class variable.
    """
...
class dataFrameOutputParser(cosmapOutputParser):
        """
        This parser takes a dictionary of key-value pairs that correspond to a single row in a DataFrame,
        and appends it to a DataFrame. In general, appending is a very expensive operation, so this parser
        ...
        def initialize(self, columns: list, dtypes: list):
            if dtypes is not None:
                ...
                self.series = {c: np.empty(self.chunksize, dtype = self.dtypes[c]) for c in columns}
        ...
        def append(self, data: dict):
            ...
            for column, value in data.items():
                if column not in self.columns:
                    raise cosmapParserException(f"Parsed data contains column {column}, which is not in the list of columns!")
