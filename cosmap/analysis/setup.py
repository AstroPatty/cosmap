...
def handle_setup(parameters: BaseModel, transformations: dict):
    """
    This function is responsible for running the setup blocks. It will run the setup blocks in the order
    specified in the analysis parameters. It will also check to make sure that all of the required parameters
    are present.
    """
...
def run_setup(parameters, dependency_graph, transformation_objects, task_order):
    ...
    output_transformations = [tname for tname in dependency_graph.nodes if dependency_graph.out_degree(tname) == 0]
    output_transformations.extend([tname for tname in dependency_graph.nodes if analysis_parameters.transformations[block_name][tname].get("output")])
    output_transformations = set(output_transformations)
    output = {}
    if output_transformations:
        for tname in output_transformations:
            if (output_name := analysis_parameters.transformations[block_name][tname].get("output-name")):
